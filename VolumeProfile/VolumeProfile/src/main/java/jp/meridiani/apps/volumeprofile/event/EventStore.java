...
public class EventStore {
	...
	public ArrayList<Event> listEvents() {
		...
		try {
			while (listCur.moveToNext()) {
				UUID uuid = UUID.fromString(listCur.getString(listCur.getColumnIndex(COL_EVENTID)));
	...
	private void loadEventData(Event event) {
		...
		Cursor dataCur = mDB.query(DATA_TABLE_NAME, null, COL_EVENTID + "=?", new String[]{uuid}, null, null, null);
		try {
	...
	private Event loadEventInternal(UUID eventId) throws EventNotFoundException {
		Event event = null;
		Cursor listCur = mDB.query(LIST_TABLE_NAME, null, COL_EVENTID + "=?", new String[] {eventId.toString()}, null, null, null);
		try {
		...
		try {
			...
			// update/insert list
			{
				...
				int rows = mDB.update(LIST_TABLE_NAME, values,
						...
						new String[]{event.getId().toString()});
				if (rows < 1) {
					values.put(COL_EVENTID, event.getId().toString());
					mDB.insert(LIST_TABLE_NAME, null, values);
				}
			}
			// update/insert data
			String[][] keykeys = new String[2][];
			keykeys[0] = event.Context().getKeys();
			keykeys[1] = event.Action().getKeys();
			for (String[] keys : keykeys) {
				for (String key : event.Context().getKeys()) {
					values.clear();
					values.put(COL_VALUE, event.getValue(key));
					int rows = mDB.update(DATA_TABLE_NAME, values,
							String.format("%1$s=? and %2$s=?", COL_EVENTID, COL_KEY),
							new String[]{event.getId().toString(), key});
					if (rows < 1) {
						values.put(COL_EVENTID, event.getId().toString());
	...
	public void deleteEvent(UUID eventId) {
		...
		try {
			...
			// delete data
			mDB.delete(DATA_TABLE_NAME, COL_EVENTID +"=?", new String[]{eventId.toString()});

			// delete list
			mDB.delete(LIST_TABLE_NAME, COL_EVENTID +"=?", new String[]{eventId.toString()});
	...
	public void readFromText(BufferedReader rdr) throws IOException, InstantiationException, IllegalAccessException {
		...
		while ((line = rdr.readLine()) != null) {
			if (EVENTS_START.equals(line)) {
	            Event event;
	            while ((event = Event.createFromText(Event.class, rdr)) != null) {
	            	storeEvent(event);
	            }
			}
		}
